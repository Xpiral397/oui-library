...
import { InitialData, User } from "@/app/context/type";
...
export default function SiginCar() {
  ...
  useEffect(() => {
    ...
    setStore(store);
    if ((store?.auth as any)?.accessToken) {
      ...
      window.location.href = window.location.origin + "/dashboard/discover";
    }
    if (has) {
