...
function LendBooksPage() {
  ...
  useEffect(() => {
    const fetchLendBooks = async () => {
      try {
        const response = await api.get("/routes/get_lent_books/");
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLendBooks([...lendBooks]); // Force re-render to update countdown timers
    }, 1000);
  ...
  return (
    <div className="relative p-4 flex flex-col items-center">
      <header className="text-center my-4">
        <h1 className="text-2xl font-bold">Library Borrowed Books</h1>
        <p className="text-sm text-gray-600">
          Check the list of borrowed books and their due dates.
        </p>
      </header>
      <div className="w-full flex justify-center">
        <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-6">
          {lendBooks.map((lendBook) => {
            const fee = calculateFee(lendBook.due_date);
            const countdown = calculateCountdown(lendBook.due_date);
            return (
              <div
                key={lendBook.id}
              ...
              >
                <h3 className="text-xl font-semibold">{lendBook.book.title}</h3>
                <p className="text-md text-gray-700">
                  Author: {lendBook.book.author}
                </p>
                <p className="text-md">Borrowed: {lendBook.start_date}</p>
                <p className="text-md">Due: {lendBook.due_date}</p>
                {fee > 0 ? (
